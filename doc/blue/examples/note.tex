...
\begin{tikzpicture}[scale=2]
  ...
  \node[thy] (prime) at (-1.5,1) {$\mmtthy{prime}{prim:\bbN\rightarrow \bbB}{prim(x)\Leftrightarrow \not\exists i<n,m<x.n\cdot m=x}$};
  \node[thy] (ps) at (1,0) {$\mmtthy{propset}{S:\cn{tpye}, p:S\rightarrow \bbB}{}$};
  \node[thy] (neprop) at (2.5,1) {$\mmtthy{neprop}{n:S}{\cn{ne}:p(n)}$};
  \node[thy] (pne) at (1,2) {$\mmtthy{neprim}{}{}$};
  \draw[conservative] (arith) -- (prime);
  \draw[include] (prime) -- (pne);
  \draw[view] (pne) to[bend left=30] node[right] {$n:=13$} (arith);
  \draw[include] (ps) -- (neprop);
  \draw[view] (ps) -- node[above,at start]{$\phi$}(prime);
  \draw[view] (neprop) -- node[above]{$\phi$}(pne);
  \npushout[.8]{ps}{pne};
\end{tikzpicture}
...
%  LocalWords:  flexiformalize realized dmesmgm14
