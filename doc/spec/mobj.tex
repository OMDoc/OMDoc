...
\svnInfo $Id: mobj.tex 8425 2009-07-17 11:59:54Z kohlhase $
...
concepts, the content dictionaries.  \inlinedef{{\twindefalt{These}{content}{dictionary}}
...
Objects and concepts that carry meaning independent of the local context (\inlinedef{they
  are called {\defin{symbol}s} in {\openmath}}) are represented as
...
\begin{definition}[id=ombind.def]
  ...
  ``{\twintoo{binding}{operator}}''\footnote{%\label {foot:binding-operator}
    ...
    content dictionary must have the value {\attval{binder}{role}{symbol}} for the
...
\begin{definition}[id=omattr.def]
  ...
  {\defin{value}s} of the keys specified by their immediately preceding siblings. In the
...
must obey the global acyclicity constraint below, \inlinedef{where we say that an
  {\openmath} element {\defin{dominate}s} all its children and all elements they
  dominate}; The {\element[ns-elt=om]{OMR}} also dominates its
\inlinedef{{\defin{target}}\footnote{The target of an {\openmath} element with an
    {\attributeshortcomment{id}{in {\sc{OpenMath}} objects}} attribute is defined
    analogously}, i.e. the element that carries the {\attributeshortcomment{id}{in
      {\sc{OpenMath}} objects}} attribute pointed to by the
...
\begin{axiom}[title={\openmath} Acyclicity Constraint]
...
\begin{definition}[display=flow,id=mml.basic.def]
  The basic objects ({\mathml} calls them {\twindef{token}{element}s}) are
\begin{description}
  \item[{\defin{identifier}s}] (element {\eldef[ns-elt=m]{ci}}) corresponding to
  ...
  \item[{\defin{number}s}] (element {\eldef[ns-elt=m]{cn}}) for number
  ...
  \item[{\defin{symbol}s}] (element {\eldef[ns-elt=m]{csymbol}}) for arbitrary
...
{\inlinedef{We say that an {\element[ns-elt=om]{OMBIND}} element {\defin{binds}} an
