...
\begin{definition}[id=symbol.def]
  ...
  and \attribute{local}{scope}{symbol}, and allows a simple specification of visibility
  conditions: if the \attribute{scope}{symbol} attribute of a \element{symbol} has
  value \attribute{local}{scope}{symbol}, then it is not \twinalt{exported}{export}{symbol}
...
\begin{description}
\item[\attribute{binder}{role}{symbol}] The symbol may appear as a binding symbol of an
...
\item[\attribute{attribution}{role}{symbol}] The symbol may be used as key in an
  ...
  object.
\item[\attribute{semantic-attribution}{role}{symbol}] This is the same as
...
\attribute{object}{role}{symbol} is assumed.
...
\begin{definition}[id=definition.def]
  ...
  \attribute{simple}{type}{definition}\ednote{maybe better leave it out}.  {\Mylstref{one}}
...
\begin{myfig}{assertion-types}{Types of Mathematical Assertions}
  \begin{tabular}{|l|l|}\hline
    ...
    \attribute{theorem}{type}{assertion}, \attribute{proposition}{type}{assertion} 
    ...
    \attribute{lemma}{type}{assertion} & a less important assertion with a proof\\\hline
    ...
    \attribute{corollary}{type}{assertion} & a simple consequence\\\hline
    ...
    \attribute{postulate}{type}{assertion}, \attribute{conjecture}{type}{assertion}
    ...
    \attribute{false-conjecture}{type}{assertion} 
    ...
    \attribute{obligation}{type}{assertion}, \attribute{assumption}{type}{assertion} 
    ...
    \attribute{formula}{type}{assertion} & if everything else fails\\\hline
...
\begin{scriptsize}
  \begin{tikzpicture}
    \node (sat) at (1,4) {\attribute{satisfiable}{status}{assertion}};
    \node (csat) at (6,4) {\attribute{counter-satisfiable}{status}{assertion}};
    \node (thm) at (1,3) {\attribute{theorem}{status}{assertion}};
    \node (cthm) at (6,3) {\attribute{counter-theorem}{status}{assertion}};
    \node (tcon) at (0,2.25) {\attribute{tautologous-conclusion}{status}{assertion}};
    \node (eqv) at (2.2,1.75) {\attribute{equivalent}{status}{assertion}};
    \node (noc) at (3.6,3.5) {\attribute{no-consequence}{status}{assertion}};
    \node (cax) at (3.6,2.5) {\attribute{contradictory-axioms}{status}{assertion}};
    \node (ceqv) at (4.7,1.75) {\attribute{counter-equivalent}{status}{assertion}};
    \node (ucon) at (7,2.25) {\attribute{unsatisfiable-conclusion}{status}{assertion}};
    \node (taut) at (1,1) {\attribute{tautology}{status}{assertion}};
    \node (usat) at (6,1) {\attribute{unsatisfiable}{status}{assertion}};
    \draw (sat) -- (noc);
    \draw (sat) -- (thm);
    \draw (csat) -- (noc);
    \draw (csat) -- (cthm);
...
\item[\attribute{type}{example}] specifying the aspect, the value is one of
  \attribute{for}{type}{example} or \attribute{against}{type}{example}
...
\element{omtext} element with the value \attribute{theorem}{type}{attribute} for the
...
\item[\attribute{type}{imports}] This optional attribute can have the values
  \attribute{global}{type}{imports} and \attribute{local}{type}{imports} (the former is
...
\begin{omtext}
  ...
  \attribute{official}{cdstatus}{theory} (i.e. approved by the {\openmath} Society),
  \attribute{experimental}{cdstatus}{theory} (i.e. under development and thus liable to
  change), \attribute{private}{cdstatus}{theory} (i.e. used by a private group of
  {\openmath} users) or \attribute{obsolete}{cdstatus}{theory} (i.e. only for archival
