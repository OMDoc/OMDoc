...
manipulated by the user or mathematical software systems. \inlinedef{We call these
  documents \defii{active}{document}s}.  For this we have to solve two problems: an
...
\begin{definition}[id=data.def]
  ...
  specifies whether the \element{data} content (value \attribute{local}{original}{data})
  or the external resource (value \attribute{external}{original}{data}) is the
  ...
  \attribute{image/jpeg}{format}{data} or \attribute{image/gif}{format}{data} for image
  data, \attribute{text/plain}{format}{data} for text data, \attribute{binary}{format}{data}
...
storing code in \omdoc documents. For a simple example see {\mylstref{omlet1}}.
...
values \attribute{execute}{action}{omlet} (pass the data to the appropriate plug-in or
execute the program code), \attribute{display}{action}{omlet} (display it to the user in
audio- or visual form), and \attribute{other}{action}{omlet} (the action is left
unspecified).
...
\attribute{new}{show}{omlet} (display the object in a new document),
\attribute{replace}{show}{omlet} (replace the current document with the presentation of the
external object), \attribute{embed}{show}{omlet} (replace the \element{omlet} element
with the presentation of the external object in the current document), and
\attribute{other}{show}{omlet} (the presentation is left unspecified).
...
\attribute{onPresent}{action}{omlet} (when the presentation document is generated),
\attribute{onLoad}{action}{omlet} (when the user loads the presentation document),
\attribute{onRequest}{action}{omlet} (when the user requests it, e.g. by clicking in the
presentation document), and \attribute{other}{action}{omlet} (the timing is left
unspecified).
...
\attribute{display}{action}{omlet}, the code would be displayed).
...
\item[\attribute{valuetype}{param}] This attribute specifies the type of the
  ...
  \attribute{data}{valuetype}{param} (the default) means that the value of the
  \attribute{value}{param} will be passed to the plug-in as a string.  The value
  \attribute{ref}{valuetype}{param} specifies that the value of the
  ...
  stored. Finally, the value \attribute{object}{valuetype}{param} specifies that
