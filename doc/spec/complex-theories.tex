...
\begin{definition}[id=morphism.def]
  ...
  (value \attribute{recursive}{type}{morphism}) or pattern-defined (value
  \attribute{pattern}{type}{morphism}).
\end{definition}
...
\begin{omtext}
  ...
  the values \attribute{global}{type}{imports} (the default) and
  \attribute{local}{type}{imports}. In the latter case, only the theory-constitutive
...
\begin{definition}[id=axiom-inclusion.def]
  ...
  \defii{local}{chain} (with morphism
...
from a \inlinedef{\defi{decomposition} for $\sigma$, if the $\sigma_\cX$ and $\sigma$
...
\begin{definition}[id=path-just.def]
  ...
  elements. \adefii{Local chains}{local}{chain} are encoded in the empty
